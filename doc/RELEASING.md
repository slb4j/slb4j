...
than snapshot instrumentation. It then asks a second time before pushing that commit. Answering yes to the second
prompt is the explicit authorization to start the release: the push to the protected release branch triggers the
protected GitHub Actions workflow. Answering no leaves the prepared plan committed locally, with no release started.
...
This updates `gradle/release-state.toml`, advances `projectVersion` in `gradle/libs.versions.toml` to the next patch
...
Normal development continues to use the `projectVersion` snapshot in `gradle/libs.versions.toml`. Preparing a release
