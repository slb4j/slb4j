...
pluginManagement {
    val versionsPluginVersion = Regex("""(?m)^\s*versions-plugin\s*=\s*"([^"]+)"""")
        .find(file("gradle/libs.versions.toml").readText())!!.groupValues[1]
    plugins {
...
if (preparedRelease != null && developmentVersion != preparedRelease.bomVersion) {
    throw GradleException(
        "prepared release ${preparedRelease.bomVersion} requires gradle/libs.versions.toml projectVersion " +
